...
export default function MessagesPage() {
  ...
  const handleSend = async () => {
    if (!selectedSession || !recipient) {
      setResult({ success: false, message: "Select a session and enter recipient" });
      return;
    }
    ...
    try {
      switch (messageType) {
        case "text":
          ...
          await sendTextMessage(selectedSession, { to: recipient, text: textMessage });
          break;
        case "image":
          if (!mediaUrl) throw new Error("Enter image URL");
          await sendImageMessage(selectedSession, { to: recipient, url: mediaUrl, caption });
          break;
        default:
