...
func (s *EventService) handleConnected(ctx context.Context, session *model.Session) {
	...
	if session.Client.Store.ID != nil {
		...
		if err := s.database.Sessions.UpdateJID(ctx, session.SessionId, jid, phone); err != nil {
			logger.Error().Err(err).Str("session", session.SessionId).Msg("Failed to update session JID/phone")
		}
	}

	if err := s.database.Sessions.UpdateStatus(ctx, session.SessionId, "connected"); err != nil {
...
func (s *EventService) handleDisconnected(ctx context.Context, session *model.Session) {
	...
	if err := s.database.Sessions.UpdateStatus(ctx, session.SessionId, "disconnected"); err != nil {
		logger.Warn().Err(err).Str("session", session.SessionId).Msg("Failed to update session status")
	}
	...
	if session.Client != nil {
		...
		if session.Client.Store != nil {
			...
			if err := session.Client.Store.Delete(ctx); err != nil {
				logger.Warn().Err(err).Str("session", session.SessionId).Msg("Failed to delete device store")
			}
		}
	}

	// Clear device JID in database
	if err := s.database.Sessions.UpdateJID(ctx, session.SessionId, "", ""); err != nil {
		logger.Warn().Err(err).Str("session", session.SessionId).Msg("Failed to clear device JID")
	}
...
func (s *EventService) handleMessage(ctx context.Context, session *model.Session, e *events.Message) {
	...
	if _, err := s.database.Messages.Save(ctx, msg); err != nil {
		logger.Warn().Err(err).Str("session", session.SessionId).Str("messageId", e.Info.ID).Msg("Failed to save message")
	}
	...
	if media := s.extractMediaInfo(session.ID, e.Info.ID, e.Message); media != nil {
		if _, err := s.database.Media.Save(ctx, media); err != nil {
			logger.Warn().Err(err).Str("session", session.SessionId).Str("messageId", e.Info.ID).Msg("Failed to save media info")
		} else {
...
func (s *EventService) handleHistorySync(ctx context.Context, session *model.Session, e *events.HistorySync) {
	...
	s.saveHistorySyncToJSON(session.SessionId, e, syncType, chunkOrder)
	...
	if s.historySyncService != nil {
		go func() {
			...
			if err := s.historySyncService.ProcessHistorySync(hsCtx, session.ID, e); err != nil {
				logger.Warn().Err(err).Str("session", session.SessionId).Msg("Failed to process history sync metadata")
			}
		}()
	}
...
func (s *EventService) handleTemporaryBan(ctx context.Context, session *model.Session, e *events.TemporaryBan) {
	...
	if err := s.database.Sessions.UpdateStatus(ctx, session.SessionId, "banned"); err != nil {
		logger.Warn().Err(err).Str("session", session.SessionId).Msg("Failed to update session status")
	}
...
func (s *EventService) handleClientOutdated(ctx context.Context, session *model.Session, e *events.ClientOutdated) {
	...
	if err := s.database.Sessions.UpdateStatus(ctx, session.SessionId, "outdated"); err != nil {
		logger.Warn().Err(err).Str("session", session.SessionId).Msg("Failed to update session status")
	}
