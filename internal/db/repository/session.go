...
func (r *SessionRepository) Create(ctx context.Context, sessionId string, apiKey string) (*model.SessionRecord, error) {
	...
	if apiKey != "" {
		query = `
			INSERT INTO "onWappSession" ("session", "apiKey") 
			VALUES ($1, $2) 
		...
		query = `
			INSERT INTO "onWappSession" ("session") 
			VALUES ($1) 
			ON CONFLICT ("session") DO UPDATE SET "updatedAt" = CURRENT_TIMESTAMP 
			RETURNING ` + sessionSelectFields
...
func (r *SessionRepository) GetByID(ctx context.Context, id string) (*model.SessionRecord, error) {
	var s model.SessionRecord
	err := r.pool.QueryRow(ctx, `SELECT `+sessionSelectFields+` FROM "onWappSession" WHERE "id" = $1`, id).
		Scan(&s.ID, &s.Session, &s.DeviceJID, &s.Phone, &s.Status, &s.ApiKey, &s.CreatedAt, &s.UpdatedAt)
	if err != nil {
		return nil, err
	}
...
func (r *SessionRepository) GetBySessionId(ctx context.Context, sessionId string) (*model.SessionRecord, error) {
	var s model.SessionRecord
	err := r.pool.QueryRow(ctx, `SELECT `+sessionSelectFields+` FROM "onWappSession" WHERE "session" = $1`, sessionId).
		Scan(&s.ID, &s.Session, &s.DeviceJID, &s.Phone, &s.Status, &s.ApiKey, &s.CreatedAt, &s.UpdatedAt)
	if err != nil {
		return nil, err
	}
...
func (r *SessionRepository) GetByApiKey(ctx context.Context, apiKey string) (*model.SessionRecord, error) {
	var s model.SessionRecord
	err := r.pool.QueryRow(ctx, `SELECT `+sessionSelectFields+` FROM "onWappSession" WHERE "apiKey" = $1`, apiKey).
		Scan(&s.ID, &s.Session, &s.DeviceJID, &s.Phone, &s.Status, &s.ApiKey, &s.CreatedAt, &s.UpdatedAt)
	if err != nil {
		return nil, err
	}
...
func (r *SessionRepository) UpdateJID(ctx context.Context, sessionId, jid, phone string) error {
	_, err := r.pool.Exec(ctx, `
		UPDATE "onWappSession" SET "deviceJid" = $1, "phone" = $2, "updatedAt" = CURRENT_TIMESTAMP 
...
func (r *SessionRepository) UpdateStatus(ctx context.Context, sessionId, status string) error {
	_, err := r.pool.Exec(ctx, `
		UPDATE "onWappSession" SET "status" = $1, "updatedAt" = CURRENT_TIMESTAMP 
		WHERE "session" = $2`, status, sessionId)
	return err
}

func (r *SessionRepository) Delete(ctx context.Context, sessionId string) error {
	_, err := r.pool.Exec(ctx, `DELETE FROM "onWappSession" WHERE "session" = $1`, sessionId)
	return err
}
...
func (r *SessionRepository) GetAllPaginated(ctx context.Context, limit, offset int) ([]model.SessionRecord, error) {
	baseQuery := `SELECT ` + sessionSelectFields + ` FROM "onWappSession" ORDER BY "id"`
...
func (r *SessionRepository) Count(ctx context.Context) (int, error) {
	var count int
	err := r.pool.QueryRow(ctx, `SELECT COUNT(*) FROM "onWappSession"`).Scan(&count)
	return count, err
}
