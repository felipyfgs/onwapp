...
func (r *ChatRepository) Save(ctx context.Context, c *model.Chat) (string, error) {
	...
	err := r.pool.QueryRow(ctx, `
		...
		ON CONFLICT ("sessionId", "chatJid") DO UPDATE SET
			"name" = COALESCE(EXCLUDED."name", "zpChats"."name"),
			...
			"conversationTimestamp" = GREATEST(EXCLUDED."conversationTimestamp", "zpChats"."conversationTimestamp"),
			"pHash" = COALESCE(EXCLUDED."pHash", "zpChats"."pHash"),
