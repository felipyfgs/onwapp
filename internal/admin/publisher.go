...
func (p *Publisher) PublishTicketCreated(ctx context.Context, session *model.Session, ticketID, contactJid, contactName, status string) {
	if p == nil || !p.IsConnected() {
		return
	}
	p.publishTicketEvent(ctx, "ticket.created", session, ticketID, contactJid, contactName, status, nil)
}

func (p *Publisher) PublishTicketUpdated(ctx context.Context, session *model.Session, ticketID, contactJid, contactName, status string, data interface{}) {
	if p == nil || !p.IsConnected() {
		return
	}
	p.publishTicketEvent(ctx, "ticket.updated", session, ticketID, contactJid, contactName, status, data)
}

func (p *Publisher) PublishTicketClosed(ctx context.Context, session *model.Session, ticketID, contactJid, contactName string) {
	if p == nil || !p.IsConnected() {
		return
	}
	p.publishTicketEvent(ctx, "ticket.closed", session, ticketID, contactJid, contactName, "closed", nil)
}

func (p *Publisher) PublishTicketMessage(ctx context.Context, session *model.Session, ticketID, contactJid, messageID string, isFromMe bool, data interface{}) {
	if p == nil || !p.IsConnected() {
		return
	}
	...
	if err != nil {
		logger.Admin().Warn().Err(err).Msg("Failed to marshal ticket message event")
		return
	}

	subject := p.client.Subject(StreamAdmin, "ticket.message")
	if _, err := p.client.JetStream().Publish(ctx, subject, eventData); err != nil {
		logger.Admin().Warn().Err(err).Str("session", session.Session).Msg("Failed to publish ticket message event")
	}
}

func (p *Publisher) publishTicketEvent(ctx context.Context, eventType string, session *model.Session, ticketID, contactJid, contactName, status string, data interface{}) {
	event := &TicketEvent{
		Event:       eventType,
		SessionID:   session.ID,
		Session:     session.Session,
	...
	eventData, err := json.Marshal(event)
	if err != nil {
		logger.Admin().Warn().Err(err).Msg("Failed to marshal ticket event")
		return
	}

	subject := p.client.Subject(StreamAdmin, eventType)
	if _, err := p.client.JetStream().Publish(ctx, subject, eventData); err != nil {
		logger.Admin().Warn().Err(err).Str("session", session.Session).Str("event", eventType).Msg("Failed to publish ticket event")
	}
